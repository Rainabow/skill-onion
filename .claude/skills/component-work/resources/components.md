...
- SkillRevealPanel: 양파 껍질 제거 후 스킬 카테고리명 + 키워드 칩 목록 표시. AnimatePresence mode="wait" + stagger (`components/card/SkillRevealPanel.jsx`)
...
- OnionVisualization: Three.js + @react-three/fiber 기반 3D 클레이 양파. LatheGeometry + Canvas 기반 matcap 텍스처(MeshMatcapMaterial), 레이어별 color 틴팅, 마우스 parallax tilt, 껍질 peel 애니메이션 (`components/media/OnionVisualization.jsx`)

## 5. Data Display — 구조화된 데이터 시각화

- LayerProgressBar: 양파 레이어 진행도 표시. Indicator(fraction) + Indicator(progress) 조합, N/Total Layers Peeled (`components/data-display/LayerProgressBar.jsx`)
...
- CursorFollower: 원형 커스텀 커서 + 레이블. useMotionValue + useSpring 기반, 데스크탑 전용, targetRef 영역 내에서만 활성화 (`components/motion/CursorFollower.jsx`)
...
## Templates — 페이지/섹션 오케스트레이터

- MediaGridGallery: 미디어 그리드 갤러리 (`components/templates/MediaGridGallery.jsx`)
- UploadModal: 업로드 모달 (`components/templates/UploadModal.jsx`)
- FilterBar: 필터 바 (`components/templates/FilterBar.jsx`)
- OnionHeroSection: 양파 인터랙션 히어로 섹션 루트. h1("Unpeeling my Skillset") 상단 중앙 정렬, 완료 시 h1 숨김. Radial glow + SVG 등고선 배경 레이어. 혼합 타이포그래피: Outfit 900 기반, 'Un'·'S'는 Playfair Display italic(각 500·900), 'Un'에 SVG 타원 서클 데코. 완료 상태 CTA 수직 중앙 배치. OnionVisualization + SkillRevealPanel + CursorFollower(양파 영역 한정) 조합, 사운드 토글/리셋 포함 (`components/templates/OnionHeroSection.jsx`)

---

## Common (유틸리티)

- Indicator: 범용 인디케이터. fraction/progress variant, current/total props, 게이지 바 (`common/ui/Indicator.jsx`)
